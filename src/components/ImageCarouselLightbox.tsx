...
export default function ImageCarouselLightbox({
  ...
  const onTouchMove = (e: React.TouchEvent) => {
    ...
    // read, then only lock 'vertical' when the gesture is clearly (2.5x) more
    ...
    // (the residual "bounce" was a near-horizontal swipe locking vertical, giving
    // a small downward pull that then snapped back).
    if (s.lock === null && (Math.abs(dx) > 10 || Math.abs(dy) > 10))
      s.lock = Math.abs(dy) > Math.abs(dx) * 2.5 ? 'v' : 'h'
    if (s.lock === 'h') {

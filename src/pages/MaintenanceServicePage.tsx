...
export default function MaintenanceServicePage() {
  ...
  // otherwise a half-filled history reads as though it cost less than it did.
  const serviceCount = sessions.length
  ...
  // overflows. With a short history the container shrinks to its content, so
  // the totals sit right under the last row instead of after a tall empty box,
  ...
  return (
    <div style={{ height: '100dvh', position: 'relative', overflow: 'hidden', fontFamily: FONT_UI }}>
      ...
      <div style={{
        position: 'absolute', inset: 0,
        background: 'linear-gradient(160deg, rgba(165,180,195,0.82) 0%, rgba(125,142,158,0.70) 50%, rgba(0,0,0,0) 100%)',
        clipPath: 'url(#svcLeftPanel)',
        pointerEvents: 'none',
      }} />
      ...
      <div ref={listRef} style={{
        ...
        // are always on screen, however long the history gets. maxHeight (not
        // height) so a short history collapses to its own size.
        maxHeight: '64dvh',
        ...
        {!loading && noCar && (
          <div style={{ padding: '40px 20px', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 }}>
            <div style={{ fontFamily: FONT_UI, fontSize: 13, color: 'rgba(212,184,106,0.45)', letterSpacing: '0.06em' }}>No car in the garage. Add one from My Cars first.</div>
            <button onClick={() => navigate('/garage/cars')} style={{ padding: '8px 18px', borderRadius: 10, border: '1px solid rgba(212,184,106,0.4)', background: 'none', color: 'rgba(212,184,106,0.85)', fontFamily: FONT_UI, fontWeight: 700, fontSize: 12, letterSpacing: '0.06em', cursor: 'pointer', WebkitTapHighlightColor: 'transparent' }}>
              My Cars
            </button>
          </div>
        )}
      ...
      {!loading && !noCar && serviceCount > 0 && (
        <div style={{
          position: 'relative', zIndex: 6,
          margin: '0 16px', paddingTop: 14,
          borderTop: '1px dashed rgba(212,184,106,0.28)',
          display: 'flex', alignItems: 'flex-start', gap: 28,
        }}>
          <div>
            <div style={{ fontFamily: FONT_UI, fontWeight: 700, fontSize: 9.5, letterSpacing: '0.16em', textTransform: 'uppercase', color: COLOR_TIMELINE_SERVICE, opacity: 0.6 }}>
              Services
            </div>
          ...
          <div>
            ...
            <div style={{ fontFamily: FONT_UI, fontWeight: 800, fontSize: 22, color: COLOR_TIMELINE_SERVICE, marginTop: 3, fontVariantNumeric: 'tabular-nums' }}>
              ${totalSpent.toLocaleString('en-US', { maximumFractionDigits: 0 })}
            </div>
            {costedCount < serviceCount && (

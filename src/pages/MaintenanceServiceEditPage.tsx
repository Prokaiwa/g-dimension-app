...
// Reverse of the New page's title encoding: "Category: description" | "Category"
function parseJobTitle(title: string): { category: string; description: string } {
  const idx = title.indexOf(': ')
  if (idx >= 0) {
    const cat = title.slice(0, idx)
    if (JOB_CATS.includes(cat)) return { category: cat, description: title.slice(idx + 2) }
  }
  if (JOB_CATS.includes(title)) return { category: title, description: '' }
  return { category: 'Custom', description: title }
}
...
export default function MaintenanceServiceEditPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      if (s) {
        ...
        setTotalEdited(true) // preserve the stored total; don't auto-overwrite on load
      ...
      if (j) {
        const rows = (j as { id: string; title: string; cost: number | null }[]).map(row => {
          const { category, description } = parseJobTitle(row.title)
          return { _id: uid(), category, description, cost: row.cost != null ? String(row.cost) : '' }
        })
        setJobs(rows)
      }
      if (r) {
  ...
  async function handleSave() {
    ...
    if (jobs.length > 0) {
      const { error: jobsError } = await supabase.from('jobs').insert(jobs.map(j => ({
        car_id: carId, session_id: sessionId, type: 'maintenance',
  ...
  if (loading) return (
    <div style={{ height: '100dvh', background: XP_WHITE, display: 'flex', flexDirection: 'column' }}>
      <div style={{ height: 30, background: 'linear-gradient(to bottom, #4a80be 0%, #2255b5 50%, #1e4fae 100%)' }} />
    </div>
  )
  ...
  return (
    <div style={{ height: '100dvh', display: 'flex', flexDirection: 'column', background: XP_WHITE, fontFamily: XP_FONT, overflow: 'hidden' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', gap: 2 }}>
          {[{ label: '─', bg: 'linear-gradient(to bottom, #d0cac0, #b0aaa0)', color: '#000' },
            { label: '□', bg: 'linear-gradient(to bottom, #d0cac0, #b0aaa0)', color: '#000' },
            { label: '✕', bg: 'linear-gradient(to bottom, #e07060, #b82818)', color: '#fff', action: () => navigate(detailRoute) },
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '10px 10px 4px', background: XP_WHITE }}>
        ...
        <XPGroupBox label="Receipts">
          {existingReceipts.length > 0 && (
            <div style={{ marginBottom: 8 }}>
              {existingReceipts.map(r => (
                <div key={r.id} style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4, padding: '3px 4px', background: '#f9f9f9', border: `1px solid ${XP_BORDER}` }}>
                  {r.file_type === 'image' && r.url
                    ? <img src={r.url} style={{ width: 28, height: 28, objectFit: 'cover', flexShrink: 0 }} />
                  ...
                  <span style={{ flex: 1, fontFamily: XP_FONT, fontSize: 11, color: XP_TEXT, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{r.file_name || 'Receipt'}</span>
                  <button onClick={() => removeExisting(r.id)} className="xp-btn" style={{ ...xpBtn, minWidth: 'auto', padding: '1px 7px', fontSize: 13, lineHeight: 1 }}>×</button>
                </div>
              ))}
            </div>
          )}
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            {existingReceipts.length === 0 && pendingReceipts.length === 0 && <span style={{ fontFamily: XP_FONT, fontSize: 11, color: '#888', fontStyle: 'italic' }}>No receipts attached</span>}
          </div>
        </XPGroupBox>
      ...
      <div style={{
        ...
        <button onClick={() => navigate(detailRoute)} className="xp-btn" style={xpBtn}>
          Cancel
        </button>

...
- Why it matters: `index.html` sets `viewport-fit=cover`, so in the Capacitor shell CSS `y=0` is the physical top of the screen. It is invisible in mobile Safari and the PWA (inset resolves to 0), which is how 45 of 46 headers shipped broken. Re-test with `test-results/safearea.mjs` + `GD_FORCE_INSET=1`.

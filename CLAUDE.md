...
| `066_job_mounted_on.sql` | **Applied 2026-07-02.** `jobs.mounted_on_job_id` (nullable self-FK → jobs.id, `on delete set null`, + index) — mounts one job onto another. First use: **Wheels + Tires combo** — when adding Wheels (part_type 1), an "Add Tires" section in `TuningAddPage` optionally creates a linked tire job (part_type 2 = Performance/Street or 3 = Truck/Off-Road) in the same session, `mounted_on_job_id` = the wheel job, inheriting the wheels' install date/mileage/installer. Full tire spec form via the generalized `renderSpecField(t, store)`. **Phase 1 = add-flow only**; build-sheet combined card + removal cascade + replacement prompt are Phase 2. Additive + idempotent. |

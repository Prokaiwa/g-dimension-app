...
supabase/migrations/                — Numbered SQL files 001–065
...
- **🏁 NEXT TASK — per-build link previews:** read `docs/LINK_PREVIEWS_TASK.md` before starting. Per-build `/builds/:username` OG previews need a Vercel serverless function injecting per-car OG tags into `index.html` (the SPA can't do per-page OG client-side). Full spec, data source (`public_car_profiles`), routing, and gotchas are in that doc. NOT STARTED.

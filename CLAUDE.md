...
- **SEO/findability** — `public/robots.txt` (allow all + sitemap), `public/sitemap.xml` (static public URLs), `public/og-default.png` (1200×630 default link-preview), and default OG/description meta in `index.html`. Per-build `/builds/:username` link previews still need an edge function (the SPA can't inject per-page OG) — separate task.

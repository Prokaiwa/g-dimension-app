...
| `065_job_condition.sql` | **Applied 2026-06-30.** `jobs.condition` (text, CHECK `new`\|`used`, nullable) — whether a mod/part was bought new or used. Captured via a New/Used toggle on the Add mod form (segmented, themed for parts-bin mode too) and a select on the Edit form; displayed as a Condition row on the private + public mod detail pages. Nullable so existing jobs stay unlabeled. Additive column. Idempotent. |

...
- **CSP inline-script hashes (vercel.json)** — `script-src` allowlists inline scripts BY SHA-256 HASH: two hashes belong to `marketing.html`'s two inline scripts, one to `index.html`'s pre-boot stale-chunk guard. **If you edit ANY inline `<script>` in `index.html` or `marketing.html`, you MUST recompute its hash** (`sha256` of the exact text between the script tags, base64) **and update the CSP in `vercel.json`** — otherwise the browser silently blocks the script in production and nothing obvious fails in dev (Vite serves no CSP locally). This already bit once: the chunk-guard script shipped without its hash and the pre-boot recovery was dead until a console error revealed it.

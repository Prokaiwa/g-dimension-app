...
`supabase/migrations/001_users.sql` → `072_car_transfers.sql` — run in order.
...
src/lib/carTransfers.ts             — Car ownership transfer helpers (migration 072, ADR-017): offer/cancel/decline via RLS-gated table writes, accept via the accept_car_transfer RPC (the app's ONLY supabase.rpc() call). Guarded carPrivate-style — pre-072 everything degrades to "no offers"

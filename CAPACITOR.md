...
dependencies (today: `@capacitor/local-notifications`, `@capacitor/app` + `@capacitor/browser` for sign-in, `@capawesome/capacitor-apple-sign-in`).
...
For a **signed release bundle** for Play (`./gradlew bundleRelease`), see
[`android/README-SIGNING.md`](./android/README-SIGNING.md). The key and its passwords
live in a gitignored `android/keystore.properties`; the whole signing block in
`app/build.gradle` is guarded on that file existing, so debug builds and CI still work
on a machine that does not hold the key.

**iOS** (needs a **Mac** with a current Xcode from the App Store; no CocoaPods, the project uses Swift Package Manager):

```bash
npm run build
npx cap sync ios       # copy dist/ + plugins, and rewrite CapApp-SPM/Package.swift
...
- **iOS is Mac-only.** Plugins come in through Swift Package Manager (`ios/App/CapApp-SPM/Package.swift`, rewritten by `npx cap sync`). A plugin whose own `Package.swift` pins `capacitor-swift-pm` to `from: "7.x"` cannot resolve against Capacitor 8; check before installing one (ADR-040).
- **iOS 27 requires the UIScene lifecycle.** An app built without it is killed at launch (SIGTRAP in `_UIApplicationEvaluateRuntimeIssueForNoSceneLifecycleAdoption`). The window and deep links live in `ios/App/App/SceneDelegate.swift`, taken from Capacitor 8.5.2's template, which is the first release with `SceneDelegateProxy`; do not downgrade below it. After bumping `capacitor-swift-pm`, delete `ios/App/build/Build` and `ModuleCache.noindex` if Xcode says `SceneDelegateProxy` is missing: the stale module cache still describes the old binary.
- **Sign-in is different on native.** Google goes through the system browser and a deep link; Sign in with Apple is iOS-only. See `supabase/migrations/AUTH_SETUP.md` §5.
- **`smallIcon: 'ic_stat_icon'`** — SHIPPED 2026-08. The drawable set now exists at
  every density under `android/app/src/main/res/drawable-*dpi/`, generated by
  `node scripts/make-notification-icon.mjs`. **Do not regenerate it with a plain
  resize.** Android draws a notification small icon from its ALPHA CHANNEL ONLY and
  repaints it flat, discarding RGB, and the G badge is a burgundy shield with the
  letter knocked out in cream, both fully opaque. Resizing the badge therefore yields a
  solid white blob with no G in it. The script thresholds on luminance so the letter
  becomes the opaque part and the shield becomes transparent. Needs macOS `sips`
  (jimp cannot decode webp) and is deliberately not part of `npm run verify`.

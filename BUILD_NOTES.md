...
3. ~~**Safe area insets**~~ ✅ **FIXED 2026-08-13 (ADR-036)** — and it was worse than the 2026-07-27 audit below estimated. That audit assumed one shared header; there is no shared header, **46 header bars are written inline across 41 files** and exactly one padded for the top inset.

   **What it actually was.** A probe across 20 routes found content inside the status-bar band on **all 20**, a tap target on **18**, and on **17 the back chevron was 100% covered** — the whole 44×44 target under the notch. With no tab bars anywhere, the `‹` chevron is the only way back, so the native build was close to unnavigable. It never showed up because it is invisible in every runtime this project can actually run: mobile Safari puts the page below the browser chrome (inset 0), and the installed PWA sets `apple-mobile-web-app-status-bar-style: black`. Both are irrelevant to Capacitor, where the `apple-mobile-web-app-*` metas are Safari web-clip hints that do nothing.

   **The fix.** `--safe-top` / `--safe-bottom` in `src/index.css`, surfaced as `SAFE_TOP` and `HEADER_HEIGHT_SAFE` in `src/tokens`. Every header uses the pair `height: HEADER_HEIGHT_SAFE, paddingTop: SAFE_TOP` — **both**, because `box-sizing: border-box` is global so padding alone eats the header's own content box. `viewport-fit=cover` stays, so the full-bleed art still reaches the screen edge.

   Two traps worth knowing, since both produced a header that looked fixed and wasn't: a `padding` **shorthand** later in the same style object silently resets `paddingTop` to 0 (10 headers did this — `paddingTop` is now the last key), and **absolutely positioned children anchor to the padding box**, so `top: 0` ignores the padding entirely (the four wedge headers — Home, Garage, Tuning, Maintenance — position everything absolutely and needed `top: SAFE_TOP, height: HEADER_HEIGHT` on each child instead).

   **How to re-test.** `npm run check:safe-area` (`scripts/safe-area-check.mjs`), which sets `GD_FORCE_INSET=1` and overrides `--safe-top` to 47px so the browser lays out exactly as a notched iPhone would, then asserts nothing interactive is left in the band. **This only works because the inset goes through a variable — `env()` cannot be overridden from a test**, which is precisely how this survived 845 commits. Three checks in `scripts/constitution.mjs` now block the regression at `npm run verify`.

   Still owed: one confirmation on a real device before submission. It is now a confirmation rather than a discovery. Note this Mac **cannot build the iOS app at all** (macOS 12; Capacitor 8's SPM project needs Xcode 15+, which needs macOS 14.5+).

   <details><summary>Original 2026-07-27 audit (superseded)</summary>

   **BOTTOM is done, TOP is essentially unhandled.**

...
- **DIY guide "Created by @handle" attribution** (user feedback, 2026-07-13) — after a car transfer, a DIY guide the previous owner wrote reads as if the new owner authored it. `diy_guides` (migration 059) has no author column — authorship is only ever derived transitively via `car_id → cars.user_id`, which is exactly what breaks on transfer. Needs a small new migration: `diy_guides.created_by` (nullable FK → `users.id`, `on delete set null`), backfilled to each existing guide's current `cars.user_id` (the only assumption available — pre-migration guides have no real authorship history), then set explicitly to `auth.uid()` on every future insert (`TuningDiyEditPage.tsx`'s create path). Display "Created by @handle" only when it differs from the car's current owner, so it stays silent for the untransferred common case.
- **"SOLD" ghost car stays visible in the seller's garage** (user feedback, 2026-07-13) — today a transferred car simply vanishes from A's `/garage/cars` carousel (the query filters `user_id = me`). Owner would like a read-only tile to persist there instead, ribboned SOLD, non-interactive except a tap that **navigates to the new owner's public build** (`/builds/:username/garage`, focused on that car) when their profile + garage are public, falling back to a static "now owned by @handle" message when private. Needs design before building: where the ghost car sorts in the carousel (probably last, after "Add another car"), whether it persists forever or only recently (a `car_transfers` join `where from_user_id = me and status = 'accepted'` has no natural cutoff), and whether it should count toward the seller's stats on `ProfilePage` (cars/mods/photos counts) — leaning no, since the underlying `cars` row genuinely isn't theirs anymore, only the transfer record is.

...
**Phase 2 (not yet built):** locked SOLD tiles on the *public* garage carousel (`PublicGaragePage`, reads the `public_sold_cars` definer view — already created in 074), and a shareable sold-car surface + OG unfurl ("‹Year Model› was sold to @B. Visit profile?"). See the What's Next entry.
...
- **SOLD ghost cars — Phase 2 (public side + sharing)** — Phase 1 (private garage keepsake + archive) shipped 2026-07-13 (migration 074, ADR-019). Still to build: (1) locked SOLD tiles on the **public** garage carousel (`PublicGaragePage`) — reads the `public_sold_cars` definer view (already created in 074); details + "Visit @B's build" redirect only, no Featured/Build Sheet/Timeline. (2) A shareable sold-car surface — a small public route (e.g. `/builds/:username/sold/:ghostId` → `PublicSoldCarPage`) showing "‹Year Model› was sold to @B. Visit their build?", plus an `api/og.js` branch so the shared link unfurls with that message.
